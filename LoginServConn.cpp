...
extern serv_info_t* g_db_server_list;
extern uint32_t		g_db_server_count;
extern uint32_t		g_db_server_login_count;
...
CLoginServConn* get_login_conn_by_handle(uint32_t handle)
{
    CLoginServConn* pConn = NULL;
/*
    hash_map<net_handle_t, CLoginServConn*>::iterator it = g_login_server_conn_map.find(handle);
    if (it != g_login_server_conn_map.end()) {
        pConn = it->second;
    }
*/

    ConnMap_t::iterator it = g_login_server_conn_map.find(handle);
    if (it != g_login_server_conn_map.end()) {
        pConn = (CLoginServConn*)it->second;
    }    
...
{
	INFO("Connecting to LoginServer %s:%d ", server_ip, server_port);
	m_serv_idx = serv_idx;
...
{
	INFO("connect to login server success ");
	m_bOpen = true;
...
{
	ERROR("login server conn onclose, from handle=%d ", m_handle);
	Close();
}
...
void CLoginServConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		ERROR("conn to login server timeout , curr_tick : %ld    m_last_recv_tick : %ld ",curr_tick, m_last_recv_tick);
		Close();
	}
}
